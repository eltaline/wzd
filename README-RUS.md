...
- Сервер не создает никаких временных файлов во время своей работы, и при этом потребляет мало оперативной памяти. Передача больших файлов осуществляется через настраиваемые полудинамические буферы малого размера на лету. В wZD не используются простые функции ctx.SendFile() и ctx.ServeContent()
...
- Разрабокта собственного репликатора и дистрибьютора + гео для возможности использования в больших системах без кластерных ФС
- Возможность полного реверсивного восстановления метаданных при их полной утере(в случае использования дистрибьютора)
- Поддержка HTTPS протокола, возможно будет поддерживаться только в будущем дистрибьюторе
