...
Получение списка всех уникальных имен файлов из директории и архива с их размерами и датами (если серверный параметр getsearch = true)
...
- **Заголовок ```WithValue``` досутупен только при условии использования заголовков ```KeysSearch``` и ```JSON``` вместе**
...
- **При использовании заголовка ```WithValue```, значения кодируются base64(снаружи) и HEX(внутри)**
