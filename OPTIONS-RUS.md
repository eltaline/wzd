...
- - show string
        --show=/path/to/file.bolt - показывается реглярные файлы/значения в bolt архиве
- -threads int
...
- -trytimes int
        trytimes=5 - количество попыток взять виртуальную блокировку для bolt архива (по умолчанию sleep = 1 между попытками), (с --pack && --list=), максимальное значение: 1000 (по умолчанию 5)
- -opentries int
        opentries=5 количество попыток открытия bolt архива, прежде чем выдать ошибку, максимальное значение: 1000 (по умолчанию 5)
- -unpack
